 ...
 var dzSparkLine = function(){
	...
	var weekLessonsChart = function(){
		if(jQuery('#weekLessonsChart').length > 0 ){
			var lessons = $.parseJSON(
				$.ajax({
					url: '/activity/api/manager_student_lessons_week',
					dataType: 'json',
					async:false,
          data: {"student_id": JSON.parse(document.getElementById('student_id').textContent)}
				}).responseText
			);
	...
	var yearLessonsChart = function(){
		if(jQuery('#yearLessonsChart').length > 0 ){
			var lessons = $.parseJSON(
				$.ajax({
					url: '/activity/api/manager_student_lessons_year',
					dataType: 'json',
					async:false,
          data: {"student_id": JSON.parse(document.getElementById('student_id').textContent)}
				}).responseText
	...
	var monthLessonsChart = function(){
		if(jQuery('#monthLessonsChart').length > 0 ){
			...
			var lessons = $.parseJSON(
				$.ajax({
					url: '/activity/api/manager_student_lessons_month',
					dataType: 'json',
					async:false,
          data: {"student_id": JSON.parse(document.getElementById('student_id').textContent)}
				}).responseText
	...
	var subjectsLessonChart = function(){
		...
		if(jQuery('#subjectsLessonChart').length > 0 ){
			var lessons = $.parseJSON(
				$.ajax({
					url: '/activity/api/manager_student_subjects_lessons',
					dataType: 'json',
					async:false,
          data: {"student_id": JSON.parse(document.getElementById('student_id').textContent)}
				}).responseText
